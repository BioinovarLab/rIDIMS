...
plot.before.rem.feat <- ggplot2::ggplot(plotDat, aes(replicate, replicate2, col = value, fill = value)) +
  ggplot2::geom_tile() +
  ggplot2::scale_fill_gradient2(low = "#F09C2B", mid= "#F09C2B", high = "#1FA5F0") +
  ggplot2::scale_color_gradient2(low = "#F09C2B", mid="#F09C2B",  high = "#1FA5F0") +
...
holder <- foreach::foreach(x=4:ncol(df.export.with.replicates), 
  ...
  # if (x == floor((20/100)*ncol(df.export.with.replicates) ) | 
  #     x == floor((50/100)*ncol(df.export.with.replicates) ) | 
  #     x == floor((80/100)*ncol(df.export.with.replicates) )){
  #     write(paste0(format(Sys.time(), "%Y-%m-%d_%H-%M-%S"),
  #     ': status 2   -  ',  formatC( ( x/ncol(df.export.with.replicates) )*100,digits=2,format="f"), "%"),
  #     file=log.file,append=TRUE)
  # }
...
plot.rem.feat <- ggplot2::ggplot(plotDat, aes(replicate, replicate2, col = value, fill = value)) +
  ggplot2::geom_tile() +
  ggplot2::scale_fill_gradient2(low = "gray", mid= "#F09C2B", high = "#1FA5F0") +
  ggplot2::scale_color_gradient2(low = "gray", mid="#F09C2B",  high = "#1FA5F0") +
  ggplot2::theme(axis.ticks.x = ggplot2::element_blank(),
        axis.text.x = ggplot2::element_blank(),
        axis.title.x = ggplot2::element_text(size=10, face="bold"),
        axis.text.y = ggplot2::element_text(size=10, face="bold"),
...
if (n.total.features.replicate.filter <  n.total.features.replicate){
...
The figure below illustrates the removal through the colors lightbrown (ion removal) and turquoise blue (ion permanence) ' , '\n')
  ...
  cat('### Dataset after procedure','   \n')
    print(plot.rem.feat)
  cat('\n')
  cat('### Dataset before procedure','   \n')
