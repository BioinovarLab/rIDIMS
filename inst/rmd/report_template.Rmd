...
mapear_sample_mz_adductmz <- function(sample_mz, adduct_df, tol = 0.001) {

  result_df <- data.frame(sample_mz = numeric(), corresponding_adductmz = numeric())

  for (i in 1:length(sample_mz)) {
    mz_target <- as.numeric(sample_mz[i])

    adduct_match <- which(abs(adduct_df$adductmz - mz_target) <= tol)

    if (length(adduct_match) > 0) {
      corresponding_adductmz <- adduct_df$adductmz[adduct_match[1]]
      result_df <- rbind(result_df, data.frame(sample_mz = mz_target, corresponding_adductmz = corresponding_adductmz))
    } else {
      result_df <- rbind(result_df, data.frame(sample_mz = mz_target, corresponding_adductmz = NA))
    }
  }
  ...
  if (input.isotopes.adducts == TRUE & samples.info$class[file] != input.subtract.group){
    ...
    for (iso_cc in 1:ChromPeaks.data.ntotal ) {
      ...
      iso_cc_scan = MSnbase::removePeaks(iso_cc_scan, 
                                         t=max(intensity(iso_cc_scan[1])[[1]]) * (input.Tresh.RA / 100))
      ...
      main_result <- summary(InterpretMSSpectrum::findMAIN(spec = current_spectra_df,ionmode = input.ion.mode,
                                                           mzabs = input.binSize, ppm = input.ppm))
      #colnames:  [1] "adductmz"          "adducthyp"         "neutral_mass"      "adducts_explained" "medppm"                   "int_perc"      #[7] "mass_score"        "int_score"         "supp_isos"         "total_score" 
    
       # Filtrar resultados não nulos de adducthyp
      filtered_result <- main_result[!is.na(main_result$adducthyp ), ]
        # Selecionar o máximo total_score
      if (nrow(filtered_result) > 0) {
        max_score_result <- filtered_result[which(filtered_result$total_score==max(filtered_result$total_score)), ]
        max_score_result$intensity <- 0
        for (adduct_cc in 1:nrow(max_score_result)  ) {
          adduct_mz = max_score_result$adductmz[adduct_cc]
          ...
          max_score_result$intensity[adduct_cc] <- adduct_mz_int
        }
      # Armazenar o resultado
      all_adducts_list[[iso_cc]] <- max_score_result
      }
    ...
    final_adducts_df <- all_adducts_df %>%
      dplyr::group_by(adductmz) %>%
      dplyr::filter(intensity == max(intensity)) %>%
      dplyr::ungroup()
    
    final_adducts_df$sample <- samples.info$sample[file]
    
    samples_adducts_list[[file]] <- final_adducts_df
    
    cat('\n\n### Adducts ', '\n\n')
    print_paged_df(final_adducts_df)
...
if (input.isotopes.adducts == TRUE){
  ...
  samples_adducts_list_consensus <-  samples_adducts_list_combined %>%
                                      dplyr::group_by(adductmz) %>%
                                      dplyr::arrange(dplyr::desc(intensity), dplyr::desc(total_score), medppm) %>%
                                      dplyr::slice(1) %>% 
                                      dplyr::ungroup()
  
  write.table(x = samples_adducts_list_consensus,
...
if (input.isotopes.adducts == TRUE){
     ...
     save(df.export,samples_adducts_list,samples_adducts_list_consensus,
          merged.status.files,
          file=paste0(data.folder, "variables_debug.Rda") 
     )
  
sample_mz <- colnames(df.export)[3:ncol(df.export)]
resultado_mapping <- mapear_sample_mz_adductmz(sample_mz, samples_adducts_list_consensus) %>% as.data.frame()
