...
if (QC_app==FALSE){
   write(paste0(format(Sys.time(), "%Y-%m-%d_%H-%M-%S"),
  ': Remove QC class from filter '),file=log.file,append=TRUE)
...
class.holder <- foreach::foreach(x=1:length(samples.filter.features$replicate),
                           .combine = "c", .packages='dplyr') %dopar% {
                             
                      item.replicate <- samples.filter.features$replicate[x]
                      
                      class.exclude.features <- samples.filter.features %>%
                        dplyr::filter(replicate == item.replicate) %>%
                        dplyr::select_if(~ . < (input.class.mean.filter/100)) %>% colnames()
                      
                      write(paste0(format(Sys.time(), "%Y-%m-%d_%H-%M-%S"),
                      ':    -  Removing ', length(class.exclude.features), " features from ", item.replicate),file=log.file,append=TRUE)
                      
                      list(c(item.replicate, class.exclude.features))
                  }
...
for(list.item in class.holder){
  if (length(list.item) == 1){
    ...
    class.exclude.features <- list.item[2:length(list.item)]
    samples.filter.features[samples.filter.features$replicate==item.replicate, class.exclude.features] <- 0
    df.class.result <- rbind(df.class.result,
                             c(item.replicate,
                              length(class.exclude.features),
                              formatC( ( length(class.exclude.features) / n.total.features.class)*100,
...
invisible(gc())
df.export <- samples.filter.features %>% select(!c("replicate", "sample"))
#clean zero cols from data -----------------------
df.export <- df.export[, colSums(df.export != 0, na.rm = TRUE) > 0] 
#-------------------------------------------------

if ( nrow(df.class.result) > 0){
  colnames(df.class.result) <- c("#", "Features removed", "Removal percentage (%)")
  print_paged_df(df.class.result) 
}else{
  cat('\n\n### No features have been removed.  \n\n') 
  df.export<-df.export.bkp
