...
mapear_sample_mz_adductmz <- function(sample_mz, adduct_df, tol = input.binSize) {

  result_df <- data.frame(sample_mz = numeric(), corresponding_adductmz = numeric(), adduct = character())

  for (i in 1:length(sample_mz)) {
    mz_target <- as.numeric(sample_mz[i])

    adduct_match <- which(abs(adduct_df$mz - mz_target) <= tol)

    if (length(adduct_match) > 0) {
      corresponding_adductmz <- adduct_df$mz[adduct_match[1]]
      corresponding_adductlabel <- adduct_df$adduct[adduct_match[1]]
      result_df <- rbind(result_df, data.frame(sample_mz = mz_target, 
                                               corresponding_adductmz = corresponding_adductmz,
                                               adduct = corresponding_adductlabel))
    } else {
      result_df <- rbind(result_df, data.frame(sample_mz = mz_target, corresponding_adductmz = NA,adduct=NA))
    }
  }
...
agrupar_sample_mz <- function(samples_df, mapping_df) {

  unique_adducts <- unique(mapping_df$corresponding_adductmz[!is.na(mapping_df$corresponding_adductmz)])

  new_adducts_df <- samples_df[, c("samples", "class"), drop = FALSE]
  
  for (adduct in unique_adducts) {
    sample_mz_subset <- mapping_df$sample_mz[mapping_df$corresponding_adductmz == adduct]
    sample_mz_subset <- as.character(na.omit(sample_mz_subset))

    valid_columns <- sample_mz_subset[sample_mz_subset %in% colnames(samples_df)]

    if (length(valid_columns) == 0) {
      next
    }

    subset_samples <- samples_df[, valid_columns, drop = FALSE]

    max_intensities <- apply(subset_samples, 1, max, na.rm = TRUE)

    new_adducts_df[[as.character(adduct)]] <- max_intensities
  }
  
  return(new_adducts_df)
}

if ( input.ion.mode=="positive"  ) {
...
cat('\n\n * Bin size: ', as.numeric(session.parameters$BinSize.PPM ) , '\n')
#cat('\n\n * Signal Noise Ratio (SNR) threshold: ', as.numeric(session.parameters$Snthresh ) , '\n')
cat('\n\n * ppm for grouping of mass peaks: ',as.numeric( session.parameters$PPM ) , '\n')
...
samples_adducts_list <- vector("list",  nrow(samples.info) )
#all_findMAIN_results <- list()

if (input.isotopes.adducts == TRUE){
  
  dir_adduts <- paste0(data.folder,"Adducts_plots/" )

  if (file.exists(dir_adduts)){
    invisible( unlink(paste0(dir_adduts,"*"), recursive = TRUE, force = TRUE) )
  }else{
    dir.create(dir_adduts)
  }
}
...
for (file in 1:nrow(samples.info) ) {
  ...
  if (input.msresolution=="high.res"){
    ...
    mfp <- xcms::MatchedFilterParam(binSize = 0.1)
    #mfp <- xcms::MatchedFilterParam(binSize = input.binSize)
    ...
    if (debug_app==TRUE){
      ...
      cat('\n\n#### Inspecting the chronogram and selecting scans based on parameters:' , '\n\n')
      cat('\n#### Intensity > ', max.int , '\n\n')
      #cat('\n#### Total Peaks > ', max.pks , '\n\n')
      ...
      p1 <- ggplot2::ggplot(temp.visual.df, ggplot2::aes(rt, int, color = factor(cluster) ) )+
        ...
        ggplot2::labs(title="",
              x ="time", y = "intensity", color = "Type") + 
        guides(color="none")+
        ggplot2::theme_bw() 
      print(p1)

    
      low_quality_scans = FALSE
      if (nrow(temp.chrom.data) < 6) {
        low_quality_scans = TRUE
      }
        
    #make replicates and/or combine:
    if (make.replicates == "TRUE" & low_quality_scans == FALSE){
    ...
    # save(ChromPeaks.data,
    #      aggregationFun,percent.limit,temp.visual.df,chrom.data,
    #      log.file,temp.chrom.data,chrom.data.item1.df,
    #      file=paste0(data.folder, "variables_step_debug.Rda") 
    #  )
      ...
      cat('\n\n### Balanced distribution of scans (in triplicate) on the chronogram: ', '\n\n')
      ...
      p2 <- ggplot2::ggplot(chrom.df, ggplot2::aes(rt, int, color = factor(cluster) ) )+
        ...
        ggplot2::scale_color_manual(values = c("0" = "lightgray", "1" = '#00AFBB', "2" = '#E7B800', 
                                         "3" = '#FC4E07'))+
        ggplot2::labs(title="",
              x ="time", y = "intensity", color = "Replicate") + 
        ggplot2::theme_bw() 
      print(p2)
      cat('\n\n')
      #, , 
      ...
      if (low_quality_scans == TRUE){
          write(paste0(format(Sys.time(), "%Y-%m-%d_%H-%M-%S"),
          ':   File with low quality scans OR incorrectly configured parameters. Unable to select minimum number of scans.'),file=log.file,append=TRUE)
        
        cat('\n\n#### ', 'Attention: File with low quality scans OR incorrectly configured parameters. Unable to select minimum number of scans.' , '\n\n')
      ...
      chrom.data.item1.df <- data.frame(cluster = 1, chrom.data.item1.df[rownames(temp.chrom.data),])
      ChromPeaks.data <- xcms::filterAcquisitionNum(ChromPeaks.data, chrom.data.item1.df$acquisitionNum)
      Biobase::fData(ChromPeaks.data)$cluster <- 1
    }

  
    # save(ChromPeaks.data,input.ppm, samples.info,
    #      session.parameters,
    #      status.files,
    #      file=paste0(data.folder, "chrompeaks.Rda"))
  
    
################### --------->>>>>

  if (input.isotopes.adducts == TRUE & samples.info$class[file] != input.subtract.group){
 
    write(paste0(format(Sys.time(), "%Y-%m-%d_%H-%M-%S"),
    ':   Grouping of Isotopes and Adducts. Ion mode = ', input.ion.mode),file=log.file,append=TRUE)
  
    #spectra_data_iso_adduct <- xcms::spectra(ChromPeaks.data)
    ChromPeaks.data.ntotal = length(ChromPeaks.data)
    # Criar uma lista para armazenar os resultados de findMAIN
    all_adducts_list <- vector("list",  ChromPeaks.data.ntotal)
    # Loop para processar cada espectro
    for (iso_cc in 1:ChromPeaks.data.ntotal ) {
      #print(iso_cc)
      
      iso_cc_scan = ChromPeaks.data[iso_cc]
      iso_cc_scan = MSnbase::removePeaks(iso_cc_scan, 
                                         t=max(MSnbase::intensity(iso_cc_scan[1])[[1]]) * (input.Tresh.RA / 100))
      iso_cc_scan <- MSnbase::clean(iso_cc_scan, all=TRUE)

      # Imprimir progresso em blocos de 10%
      if (iso_cc %% ceiling(ChromPeaks.data.ntotal / 10) == 0) {
        progresso_cc_str <- round(iso_cc / ChromPeaks.data.ntotal,2)*100
        
        write(paste0(format(Sys.time(), "%Y-%m-%d_%H-%M-%S"),
        ':   Grouping: ', progresso_cc_str, "%"),file=log.file,append=TRUE)
        
      }
      
      # Extrair o espectro atual
      current_spectrum <- xcms::spectra(iso_cc_scan)
      
      # Extrair os valores de m/z e intensidade do espectro atual
      mz_values <- MSnbase::mz(iso_cc_scan)[1][[1]]
      intensity_values <- MSnbase::intensity(iso_cc_scan)[1][[1]]
      
      # Criar um data frame para o espectro atual
      current_spectra_df <- data.frame(
        mz = mz_values,
        intensity = intensity_values
      )
      
      # Usar a função findMAIN() para analisar o espectro
      findMAIN_res <- InterpretMSSpectrum::findMAIN(spec = current_spectra_df,ionmode = input.ion.mode,
                                                           mzabs = input.binSize, ppm = input.ppm,
                                   rules = c(ads, nls), mainpkthr = 0.3,
                                   adducthyp = ads[grep("[M", ads, fixed = TRUE)],
                                   collapseResults = FALSE )
      ##mainpkthr = 0.3 - ramclustr

      
      findMAIN_res_summarytable <- summary(findMAIN_res)
      #RAMClustR summaryscores https://github.com/cbroeckl/RAMClustR
      summaryscores <- sapply(1:length(findMAIN_res), FUN = function(x) {
          is.adduct <- which(findMAIN_res[[x]][, "adduct"] %in% ads)
          det.adducts <- findMAIN_res[[x]][is.adduct, "adduct"]
          is.nl <- which(findMAIN_res[[x]][, "adduct"] %in% nls)
          keep <- is.adduct
          int <- sum(findMAIN_res[[x]][is.adduct, "intensity"])
          if (length(is.nl) > 0) {
            int <- sum(int, sum(0.5 * (findMAIN_res[[x]][is.nl, "intensity"])))
            keep <- c(keep, is.nl)
          }
          int <- int/sum(findMAIN_res[[x]][, "intensity"]) * length(is.adduct)
          if (any(grepl("[M+H-NH3]+", det.adducts)) & length(det.adducts == 
                                                             1)) {
            int <- int/2
          }
          mzerr <- (1/findMAIN_res_summarytable[x, "medppm"])^0.5
          return(int * mzerr)
      })
      best_findMAIN <- which.max(summaryscores)
      if (length(best_findMAIN) == 0) {
        best_findMAIN <- 1
      }
      sort_best_findMAIN <- sort(summaryscores, decreasing = TRUE)[1]/sort(summaryscores, 
                                                     decreasing = TRUE)[2]
      if(is.na(sort_best_findMAIN)) {sort_best_findMAIN<- 1}
      
      findMAIN_neutral_mass <- as.numeric(findMAIN_res_summarytable[best_findMAIN, "neutral_mass"])
      findMAIN_best_result <- findMAIN_res[[best_findMAIN]]
      findMAIN_best_result_noNA <- findMAIN_best_result[!is.na(findMAIN_best_result$adduct),]
      findMAIN_best_result_noNA$neutral_mass <- findMAIN_neutral_mass
      findMAIN_best_result_noNA$scan <- iso_cc

      #save(current_spectra_df,input.ion.mode,input.binSize,input.ppm,ads, nls,iso_cc,samples.info,file,
      #file=paste0(data.folder, "findMAIN_",file,"_",iso_cc,".Rda"))
      
      if (nrow(findMAIN_best_result_noNA) > 0) {
        for (adduct_cc in 1:nrow(findMAIN_best_result_noNA)  ) {
          adduct_mz = round(findMAIN_best_result_noNA$mz[adduct_cc] , 4) 
          adduct_mz_find = which( round(current_spectra_df$mz, 4) == adduct_mz)
          adduct_mz_find = adduct_mz_find[1]
          adduct_mz_int = current_spectra_df$intensity[adduct_mz_find]
          #sample_mz_val = current_spectra_df$mz[adduct_mz_find]
          findMAIN_best_result_noNA$raw_intensity[adduct_cc] <- adduct_mz_int
          #findMAIN_best_result_noNA$sample_mz[adduct_cc] <- sample_mz_val
        }
      }
      
      findMAIN_best_result_noNA$file <- samples.info$sample[file]
      
      findMAIN_best_result_noNA.temp <- findMAIN_best_result_noNA %>%
                        dplyr::group_by(neutral_mass) %>%
                        dplyr::slice_max(raw_intensity, n = 1, with_ties = FALSE) %>%
                        dplyr::ungroup()
        
        png(paste0(dir_adduts,samples.info$sample[file],"_",
                   round(findMAIN_best_result_noNA.temp$mz[1],5),"_",iso_cc,".png"), width  = 3.25*2,
        height = 3.25, units = "in",  res  = 600, pointsize = 2.5)
        par(cex.main=1, cex.lab=1,cex.axis=0.8,cex=2.5)
          InterpretMSSpectrum::PlotSpec(x = findMAIN_best_result,
                    txt = findMAIN_best_result[, c("mz", "adduct")],
                    cutoff = 0, masslab = 0,
                    ylim = c(0,  1.5 * max(findMAIN_best_result[, 2])) )
        dev.off()
      rm(findMAIN_best_result_noNA.temp)

                              
      all_adducts_list[[iso_cc]] <- findMAIN_best_result_noNA
    
    }
    
    # Concatenar todos os data frames da lista all_main_results
    all_adducts_df <- do.call(rbind, all_adducts_list)

        # Filtrar adductmz com adducthyp diferentes e selecionar o maior intensity
    final_adducts_df <- all_adducts_df %>%
      dplyr::group_by(mz) %>%
      dplyr::filter(raw_intensity == max(raw_intensity)) %>%
      dplyr::ungroup()
    

    samples_adducts_list[[file]] <- final_adducts_df

    cat('\n\n### Adducts ', '\n\n')
    print_paged_df(final_adducts_df[,c("mz","adduct","neutral_mass")])
    
    write(paste0(format(Sys.time(), "%Y-%m-%d_%H-%M-%S"),
    ':   Final: ', nrow(final_adducts_df), " adducts"),file=log.file,append=TRUE)
    
  }      
    
    #save(ChromPeaks.data,
    #     file=paste0(data.folder, "chrompeaks_",file,"_.Rda"))
     
################### --------->>>>>
    
    comSpec <- MSnbase::combineSpectra(ChromPeaks.data,
                                ppm = input.ppm,
...
if (input.isotopes.adducts == TRUE){
  ...
  #all_findMAIN_results_combined <- do.call(rbind, all_findMAIN_results)
  
  samples_adducts_list_combined <- do.call(rbind, samples_adducts_list)

  samples_adducts_consensus <- samples_adducts_list_combined %>%
                                dplyr::group_by(mz) %>%
                                dplyr::slice_max(raw_intensity, n = 1, with_ties = FALSE) %>%
                                dplyr::ungroup()
  
  samples_adducts_consensus$neutral_mass_round = round( samples_adducts_consensus$neutral_mass,4)
  
  samples_adducts_consensus <- samples_adducts_consensus %>%
                              dplyr::group_by(neutral_mass_round) %>%
                              dplyr::slice_max(raw_intensity, n = 1, with_ties = FALSE) %>%
                              dplyr::ungroup()

  write.table(x = samples_adducts_consensus,
                file = paste0(data.folder, "Samples_adducts.csv"), sep=",", row.names=FALSE, quote = F)
  #write.table(x = all_findMAIN_results_combined,
  #              file = paste0(data.folder, "All_findMAIN_results_combined.csv"), sep=",", row.names=FALSE, quote = F)
...
set.seed(12345)
        ...
        # save(data.folder,
        #      samples.info,input.binSize,
        #      status.files,samples_adducts_list,output.folder,
        #      merged.status.files,
        #      file=paste0(data.folder, "variables_step_2.Rda") 
        #  )
...
BiocParallel::register(BiocParallel::SerialParam())
processed.sps <- Spectra::Spectra(paste0(output.folder,
                                         merged.status.files$replicate.file), 
                                  backend = Spectra::MsBackendMzR())



############### -------->>> #################################
##processed.sps <- Spectra::setBackend(processed.sps, Spectra::MsBackendMemory() )
...
#prevent colnames error
if ( is.null(ncol(df.export))==TRUE ) {
  write(paste0(format(Sys.time(), "%Y-%m-%d_%H-%M-%S"),
           ':*** Unable to continue processing. Check the parameters used. *** '),file=log.file,append=TRUE)
  cat('\n\n## Error: Unable to continue processing. Check the parameters used.', '\n\n')
  knitr::knit_exit() 
}
...
### verifica numero de samples

if (nrow(samples.info) >= 2){
...
write(paste0(format(Sys.time(), "%Y-%m-%d_%H-%M-%S"),
  ': Unable to perform PCA.'  ),file=log.file,append=TRUE)
...
abline(h = -0.07, lty = 1,lwd = 1.6, col="black")
...
for (m in 1:nrow(df.export.mean)){
  Spectra::plotSpectra(obj.df.export.class.mean[m], lwd = 2, col = cores[obj.df.export.class.mean$name[m]] , main = "")
  ...
  write.table( data.frame(mz = colnames(df.export.mean)[-1] , 
             ...
             file=paste0(output.folder, "raw_data_mean_" ,as.character(df.export.mean[m,1]), ".csv"),  sep=";",row.names = F)
...
write.table(x = data_filtered,
              file = paste0(data.folder, "Data_metaboanalyst_subtraction_", input.min.fold ,"_MFC.csv"), sep=",", row.names=FALSE, quote = F)

cat('\n\n File exported to: ', paste0(data.folder, "Data_metaboanalyst_subtraction_", input.min.fold ,"_MFC.csv") ,'  \n\n') 
...
df.export.bkp <- df.export
     ...
     #     merged.status.files,
     #     file=paste0(data.folder, "variables_debug_12072023.Rda") 
     # )
...
if (length(samples.filter.features.filtered.selected) == 2){
  df.export <- df.export[,c("samples", "class")]
}else{
...
if ( n.total.classes > 0 ){
  ...
  if (ncol(df.export) > 3){
    ...
    # save(obj.df.export.class.mean,
    #      cores,
    #      df.export.mean,
    #      file=paste0(data.folder, "plot_vars_.Rda") 
    #  )
    ...
    for (m in 1:nrow(df.export.mean)){ 
      Spectra::plotSpectra(obj.df.export.class.mean[m], lwd = 2, col = cores[obj.df.export.class.mean$name[m]], main = "" )
      legend("topright", col = cores[obj.df.export.class.mean$name[m]], legend = obj.df.export.class.mean$name[m], pch = 15)
      abline(h = -0.05, lty = 1,lwd = 1.6, col="black")
      
      write.table( data.frame(mz = colnames(df.export.mean)[-1] , 
                              int = as.numeric(df.export.mean[m,2:ncol(df.export.mean)])),
                   file=paste0(output.folder, "processed_data_mean_" ,as.character(df.export.mean[m,1]), ".csv"),  sep=";",row.names = F)
  ...
  write(paste0(format(Sys.time(), "%Y-%m-%d_%H-%M-%S"),': Attention! Processing resulted in insufficient number of samples per class. Check the parameters used for correct visualization of the processed spectra.'),file=log.file,append=TRUE)
  
  cat('\n\n### Attention! Processing resulted in insufficient number of samples per class. Check the parameters used for correct visualization of the processed spectra. \n\n') 
...
# save(df.export,samples_adducts_consensus,
#     merged.status.files,
#     file=paste0(data.folder, "variables_debug.Rda") 
# )
  
sample_mz <- colnames(df.export)[3:ncol(df.export)]
resultado_mapping <- mapear_sample_mz_adductmz(sample_mz, samples_adducts_consensus) %>% as.data.frame()

write.table(resultado_mapping,
             file=paste0(data.folder, "Adducts_map", ".csv"),  sep=",",row.names = F)

df_cluster_adducts <- agrupar_sample_mz(df.export, resultado_mapping)
write.table(x = df_cluster_adducts,
            file = paste0(data.folder,"Data","_metaboanalyst_samples_adduct_filtered.csv"),
            sep=",", row.names=FALSE, quote = F)
...
write(paste0( "Resolution mode = " , as.character(session.parameters$MS ) ),file=log.file,append=TRUE)
write(paste0( "BinSize.PPM = " ,     as.character(session.parameters$BinSize.PPM ) ),file=log.file,append=TRUE)
write(paste0( "SNR treshold = " , as.character(session.parameters$Snthresh ) ),file=log.file,append=TRUE)
write(paste0( "aggregationFun = " , as.character(session.parameters$aggregationFun ) ),file=log.file,append=TRUE)
write(paste0( "Make replicates = " , as.character(session.parameters$replicates ) ),file=log.file,append=TRUE)
write(paste0( "PPM of mass peaks grouping = " , as.character(session.parameters$PPM ) ),file=log.file,append=TRUE)
write(paste0( "Filter spectrum intensity by = " , as.character(session.parameters$Tresh.RA ) ),file=log.file,append=TRUE)
write(paste0( "Name of the class to subtract = " , as.character(session.parameters$Subtract ) ),file=log.file,append=TRUE)
write(paste0( "Minimum fold change = " , as.character(session.parameters$MinFold ) ),file=log.file,append=TRUE)
write(paste0( "Filter chronogram by = " , as.character(session.parameters$Chr.limit ) ),file=log.file,append=TRUE)
write(paste0( "Class filter type = " , as.character(session.parameters$ClassFilter ) ),file=log.file,append=TRUE)
write(paste0( "Class filter value = " , as.character(session.parameters$ClassFilterValue ) ),file=log.file,append=TRUE)
write(paste0( "Replicate filter = " , as.character(session.parameters$ReplicateFilter ) ),file=log.file,append=TRUE)
write(paste0( "Replicate filter value = " , as.character(session.parameters$ReplicateFilterValue ) ),file=log.file,append=TRUE)
write(paste0( "Make heatmaps = " , as.character(session.parameters$MakeHeatmaps ) ),file=log.file,append=TRUE)
write(paste0( "Ion mode = " , as.character(session.parameters$ion_mode ) ),file=log.file,append=TRUE)
write(paste0( "Report code = " , as.character(session.parameters$ReportSerial ) ),file=log.file,append=TRUE)
write(paste0( "Data folder = " , as.character(session.parameters$Data.folder ) ),file=log.file,append=TRUE)

##############################
################# parallel Stop cluster ##
if(.Platform$OS.type=="windows"){ # If we are on windows
