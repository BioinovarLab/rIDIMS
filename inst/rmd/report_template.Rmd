...
################### --------->>>>>
  ...
  if (input.isotopes.adducts == TRUE){
    ...
    write(paste0(format(Sys.time(), "%Y-%m-%d_%H-%M-%S"),
    ':   Grouping of Isotopes and Adducts'),file=log.file,append=TRUE)
  
    spectra_data_iso_adduct <- xcms::spectra(ChromPeaks.data)

    # Criar uma lista para armazenar os resultados de findMAIN
    all_adducts_list <- vector("list", length(spectra_data_iso_adduct))
    # Loop para processar cada espectro
    for (iso_cc in seq_along(spectra_data_iso_adduct) ) {
      
      # Imprimir progresso em blocos de 10%
      if (iso_cc %% ceiling(length(spectra_data_iso_adduct) / 10) == 0) {
        progresso_cc_str <- round(iso_cc / length(spectra_data_iso_adduct),2)*100
      ...
      current_spectrum <- spectra_data_iso_adduct[[iso_cc]]
      
      # Extrair os valores de m/z e intensidade do espectro atual
      mz_values <- mz(current_spectrum)
      intensity_values <- intensity(current_spectrum)
      ...
      main_result <- summary(InterpretMSSpectrum::findMAIN(spec = current_spectra_df))
      ...
      if (nrow(filtered_result) > 0) {
        max_score_result <- filtered_result[which.max(filtered_result$total_score), ]
